...
const LANE_STYLE = {
  "01": { bg: 'oklch(0.22 0.06 270)', color: '#fff', label: '01 · Lead bet · 60% effort' },
  "02": { bg: 'oklch(0.38 0.06 20)',  color: '#fff', label: '02 · Strong second · 30% effort' },
  "03": { bg: 'var(--bg-tint)',       color: 'var(--ink-3)', label: '03 · Hedge · 10% effort' },
};
...
function RoleCard({ role, onConfidence }) {
  ...
  const lane = LANE_STYLE[role.lane] || LANE_STYLE["03"];
...
function RolesPage({ roles, setRoles }) {
  ...
  const filtered = laneFilter === "All" ? roles : roles.filter(r => r.lane === laneFilter);
  ...
  return (
    <div data-screen-label="03 Stress-Tested Roles">
      <div className="page-h">
        <div>
          <div className="eyebrow">5 roles · 3 lanes · stress-tested against the positioning</div>
          <h1>Stress-Tested Roles</h1>
          <div className="sub" style={{ maxWidth: 640 }}>Three lanes, six criteria. Each role is mapped against required skills, current mastery, and how comfortable I'd feel walking into the interview today. Effort split: 60 / 30 / 10.</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 22 }}>
        {[
          { lane: "01", title: "Brand Illustrator", sub: "Freelance · Fiverr · Behance · Upwork", effort: "60%", income: "₹40k–1L/mo", competition: "Crowded but niche — style is the moat", growth: "Compounds · name builds reach", bg: 'oklch(0.22 0.06 270)', color: '#fff' },
          { lane: "02", title: "Brand Designer · Studio", sub: "Illustration-led studios", effort: "30%", income: "₹6–12 LPA", competition: "Thin pool — fit-driven hiring", growth: "Linear · studio CV ladder", bg: 'oklch(0.38 0.06 20)', color: '#fff' },
          { lane: "03", title: "In-house D2C", sub: "Brand & content · hedge only", effort: "10%", income: "₹6–10 LPA", competition: "Heavy · many generalists", growth: "Limited · title cap at lead", bg: 'var(--bg-tint)', color: 'var(--ink)' },
        ].map(l => (
          <div key={l.lane} style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid var(--line)', boxShadow: 'var(--shadow-1)' }}>
            <div style={{ padding: '10px 14px', background: l.bg, color: l.color }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', opacity: 0.75 }}>Lane {l.lane} · {l.effort} effort</div>
              <div style={{ fontSize: 14, fontWeight: 600, marginTop: 3 }}>{l.title}</div>
              <div style={{ fontSize: 11, opacity: 0.7, marginTop: 2 }}>{l.sub}</div>
            </div>
            <div style={{ padding: '10px 14px', background: 'var(--surface)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px', fontSize: 11.5 }}>
                <div><span style={{ color: 'var(--ink-3)', fontSize: 10.5, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Income</span><b>{l.income}</b></div>
                <div><span style={{ color: 'var(--ink-3)', fontSize: 10.5, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Growth</span><b>{l.growth}</b></div>
                <div style={{ gridColumn: '1 / -1' }}><span style={{ color: 'var(--ink-3)', fontSize: 10.5, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Competition</span><b>{l.competition}</b></div>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="stats" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
        ...
        <div className="stat">
          <div className="lbl">Ready to apply</div>
      ...
      <div className="filter-bar">
        <span style={{ fontSize: 12, color: 'var(--ink-3)', marginRight: 4 }}>Lane</span>
        {["All", "01", "02", "03"].map(l => (
          <button key={l} className={`chip ${laneFilter === l ? 'active' : ''}`} onClick={() => setLaneFilter(l)}>
            {l === "All" ? "All lanes" : `Lane ${l}`}
          </button>
        ))}
        <div style={{ width: 1, height: 20, background: 'var(--line)', margin: '0 4px' }}></div>
        <span style={{ fontSize: 12, color: 'var(--ink-3)', marginRight: 4 }}>Sort</span>
